# العجلة: نتيجة من الخادم وأمان الكود
...
| البند | الحالة |
|-------|--------|
| تحقق الأدمن عبر السيرفر | ✅ Cloud Function `validateAdminToken` — الكود يُتحقق منه على السيرفر (ADMIN_CODE في env)، التحقق المحلي fallback عند عدم توفر Firebase |
| إخفاء الأسرار في الإعدادات | ✅ رقم واتساب، رابط التحقق من الأهلية، رابط ويب هوك — تُعرض مقنّعة (•••• أو "مضبوط") مع زر إظهار/إخفاء |
| قواعد Firestore | ✅ دالة `canWrite()` جاهزة — حالياً تُرجع `isAuthenticated() \|\| true` (مفتوح)؛ للتفعيل احذف `\|\| true` من `firestore.rules` بعد تفعيل Firebase Auth. انظر `docs/FIREBASE_SECURITY.md` |
...
### عقد السيرفر (checkEligibilityUrl) — مصدر الحقيقة

عند ضبط **checkEligibilityUrl** يصبح السيرفر مصدر الحقيقة لـ "هل هذا الرقم مسموح يلعب؟". الفرونت يرسل طلبين:

1. **التحقق (بدون action):**  
   `POST` body: `{ phone, sessionToken, at }`  
   الرد المتوقع: `{ allowed: boolean, message?: string, cooldownEndsAt?: number }`  
   - `cooldownEndsAt`: وقت انتهاء الحظر (مللي ثانية منذ epoch، وقت السيرفر) — للعرض في التايمر.
   - لو `allowed: false` والرقم في حظر، يُفضّل إرجاع `cooldownEndsAt` حتى يعرض الفرونت الوقت المتبقي.

2. **تسجيل لفة ناجحة (بعد انتهاء الدوران):**  
   `POST` body: `{ phone, sessionToken, at, action: "recordSpin" }`  
   - السيرفر يسجّل آخر تاريخ لفة لهذا الرقم (بوقت السيرفر) ليطبّق الحظر 15 يوم في الطلبات التالية.
   - الفرونت يستدعي هذا بعد كل لفة ناجحة؛ فشل الطلب لا يمنع تجربة المستخدم.
